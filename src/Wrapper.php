...
/**
 * Heavily inspired by David Grudl's package "dg/bypass-finals".
 */
final class Wrapper
{
    ...
    {
        $skip = str_contains($path, 'jaem3l/unfuck/src') || str_contains($path, 'nikic/php-parser/lib/PhpParser');

        $usePath = (bool) ($options & STREAM_USE_PATH);
        if ($mode === 'rb' && !$skip && pathinfo($path, PATHINFO_EXTENSION) === 'php') {
            $content = $this->native('file_get_contents', $path, $usePath, $this->context);
            if ($content === false) {
                return false;
            }
            $modified = Code::unfuck($content);
            if ($modified !== $content) {
                $this->handle = tmpfile();
